-- 1) Add 'admin' role to user_role enum safely
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 
...
-- 4) Allow company admins to view and update their company rows
DO $$ BEGIN
  IF NOT EXISTS (
...
-- 5) Credit ledger for auditing credits movements
...
-- 6) Booking credits handler function and trigger
...
-- 7) Admin read-all policies for key tables
DO $$ BEGIN
  -- Profiles
  IF NOT EXISTS (
...
-- 8) updated_at trigger for companies
DO $$ BEGIN
  IF NOT EXISTS (
